...
var _Sources = (() => {
  ...
  // src/Oredoujin/Oredoujin.ts
  var Oredoujin_exports = {};
  __export(Oredoujin_exports, {
    Oredoujin: () => Oredoujin,
    OredoujinInfo: () => OredoujinInfo
  });
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  // src/Oredoujin/Oredoujin.ts
  var DOMAIN = "https://oredoujin.com";
  var OredoujinInfo = {
  ...
  var Oredoujin = class extends MangaStream {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
    ...
    configureSections() {
      ...
      this.homescreen_sections["latest_update"].selectorFunc = ($2) => $2("div.bsx", $2("h3:contains(\u0E2D\u0E31\u0E1B\u0E40\u0E14\u0E15\u0E25\u0E48\u0E32\u0E2A\u0E38\u0E14\u0E40\u0E21\u0E37\u0E48\u0E2D)")?.parent()?.next());
      this.homescreen_sections["latest_update"].subtitleSelectorFunc = ($2, element) => $2("span.nchapter", element).first().text().trim();
    }
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const request = App.createRequest({
      ...
      this.checkResponseError(response);
      const $2 = load(response.data);
      const results = await parseSearchResults($2, this);
      const manga = [];
      for (const result of results) {
        let mangaId = result.slug;
        if (await this.getUsePostIds()) {
    ...
    async getChapters(mangaId) {
      ...
      this.checkResponseError(response);
      const $2 = load(response.data);
      return parseChapterList($2, mangaId, this);
    }
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: await this.getUsePostIds() ? `${this.baseUrl}/?p=${mangaId}/` : `${this.baseUrl}/${this.directoryPath}/${mangaId}/`,
      ...
      this.checkResponseError(response);
      const $2 = load(response.data);
      const chapters = $2("div.eplister.eplisterfull").find("li").toArray();
      if (chapters.length === 0) {
        throw new Error(`Unable to fetch chapter list for manga with mangaId: ${mangaId}`);
      }
      const chapter = chapters.find((x) => $2(x).attr("data-id") === chapterId);
      if (!chapter) {
        throw new Error(`Unable to fetch a chapter for chapter number: ${chapterId}`);
      }
      const id = $2("a", chapter).attr("href") ?? "";
  ...
  function parseSearchResults($2, source) {
    const results = [];
    for (const obj of $2("div.listupd > article.maindet > div.inmain").toArray()) {
      const slug = ($2("a", obj).attr("href") ?? "").replace(/\/$/, "").split("/").pop() ?? "";
      const path = ($2("a", obj).attr("href") ?? "").replace(/\/$/, "").split("/").slice(-2).shift() ?? "";
      if (!slug || !path) {
        throw new Error(`Unable to parse slug (${slug}) or path (${path})!`);
      ...
      const title = $2("a", obj).attr("title") ?? "";
      const image = encodeURI(decodeURI((0, import_html_entities2.decode)($2("a > img", obj).attr("src")?.trim()))) ?? "";
      const subtitle = $2("div.mdinfo > div.mdinfodet > span.nchapter > a", obj).text().trim() ?? "";
      results.push({
        slug,
        path,
        image: image || source.fallbackImage,
        title: (0, import_html_entities2.decode)(title),
        subtitle: (0, import_html_entities2.decode)(subtitle)
      });
    }
    return results;
  }
  function parseChapterList($2, mangaId, source) {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2("li", "div.eplister.eplisterfull").toArray()) {
      const title = (0, import_html_entities2.decode)($2("div.epl-title", chapter).text().trim());
      const date = convertDate($2("div.epl-date", chapter).text().trim(), source);
  ...
  function parseChapterDetails($2, mangaId, chapterId) {
    const pages = [];
    for (const page of $2("img", `#post-${chapterId} > div.bixbox.episodedl > div > div.epcontent.entry-content > center > p`).toArray()) {
      pages.push($2(page).attr("src") ?? "");
    }
    const chapterDetails = App.createChapterDetails({
      id: chapterId,
      mangaId,
  ...
  return __toCommonJS(Oredoujin_exports);
})();
